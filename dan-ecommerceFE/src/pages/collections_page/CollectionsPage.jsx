...
import { getCategorybasedProduct, getSearch, getCategorys, getSubCategories, getCategoryPopularProduct, getCategorylatestProduct, getCategoryHighToLowRateProduct, getCategoryLowToHighRateProduct } from '../../API/userApi'
...
function CollectionsPage() {
    ...
    console.log(selectedFilter, "selected filtterrrr%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5");
    ...
    // useEffect that runs when selectedFilter changes
    useEffect(() => {
        if (selectedFilter === "Popularity") {
            try {
                const popular = getCategoryPopularProduct(firstCategoryId).then((res) => {
                    setProducts(res);
                });
            } catch (error) {
                console.log(error);
            ...
            try {
                const latest = getCategorylatestProduct(firstCategoryId).then((res) => {
                    setProducts(res);
                });
            } catch (error) {
                console.log(error);
            }
        } else if (selectedFilter === "Best Rated") {
            try {
                const popular = getCategoryPopularProduct(firstCategoryId).then((res) => {
                    setProducts(res);
                });
            ...
            try {
                const popular = getCategoryHighToLowRateProduct(firstCategoryId).then((res) => {
                    setProducts(res);
                });
            } catch (error) {
                console.log(error);
            ...
            try {
                const popular = getCategoryLowToHighRateProduct(firstCategoryId).then((res) => {
                    setProducts(res);
                });
            } catch (error) {
                console.log(error);
                    ...
                    <div className={`w-full h-[4.98%]  flex justify-between  ${isCategoryOpen ? 'pl-[0%] md:pl-[34.5%] lg:pl-[33.7%]  xl:pl-[28%] pr-[0%]' : 'pl-[0%]  pr-[0%] '
                        ...
                        <div className="h-[100%] w-[40%] md:w-[27%] flex justify-between gap-[7.5%] items-center">
                            ...
                            >
                                ...
                                >
                                    <h5
                                        className={`text-[#803314] font-semibold ${isCategoryOpen
                                ...
                                <div className="hidden md:flex items-center md:h-[30%]">
                                    <p
                                        className={`font-semibold text-black ${isCategoryOpen
                                            ? 'text-[1vw]'
                                            : 'text-[1.2vw]'
                                            }`}
                                    >
                        ...
                        <div className="relative  h-full w-[39.3%]  flex justify-between items-center">
                            ...
                            {/* Sort Section Start */}
                            <button onClick={() => setIsSortOpen(!isSortOpen)} className=" w-[12%] h-full py-[1%] flex justify-center items-center shadow-xl rounded-[.5vw] cursor-pointer">
                            ...
                            {isSortOpen && (
                                <div
                                    ref={dropDownRef}
                                    className={`absolute right-0 top-full mt-[1vw] w-[100%] md:w-[64%] max-w-[100%] bg-white shadow-2xl z-10 flex items-center justify-center ${isCategoryOpen ? 'rounded-[1.5vw]' : 'rounded-[2vw]'}`} >
                                    <ul className="py-2 w-[100%] md:w-[94%] h-auto flex flex-col gap-1">
                                        <button onClick={handleClearFilter} className='bg-amber-700 text-[#fff] text-[14px] rounded-2xl cursor-pointer hover:bg-amber-800 p-1'>clear</ button>
                                        {sortOptions.map((option) => (
                                            <li
                                                key={option}
                                                onClick={() => handleSortSelect(option)}
                                                className={`px-3 py-2 cursor-pointer hover:bg-gray-100 rounded-[1vw]  font-semibold break-wordswhitespace-normal${isCategoryOpen ? 'text-[1.2vw] md:text-[13px]' : 'text-[1.5vw] md:text-[1vw]'}${selectedFilter === option
                                                    ? " shadow-md rounded-[1vw] text-[#BC7050]"
                                                    : ""
                                                    }
                                              `}
                                            >
